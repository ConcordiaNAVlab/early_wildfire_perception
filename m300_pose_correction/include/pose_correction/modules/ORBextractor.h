...
class ExtractorNode {
 public:
  ...
  // 储提取器节点的列表（其实就是双向链表）的一个迭代器,可以参考[http://
  // ww.runoob.com/cplusplus/cpp-overloading.html]
  // 个迭代器提供了访问总节点列表的方式，需要结合cpp文件进行分析
  std::list<ExtractorNode>::iterator lit;

  // 如果节点中只有一个特征点的话，说明这个节点不能够再进行分裂了，这个标志置位
  // 个节点中如果没有特征点的话，这个节点就直接被删除了
  bool bNoMore;
};
...
class ORBextractor {
 public:
   ...
   * @param[in] mask          图像掩膜，辅助进行图片处理，可以参考[https://
   * ww.cnblogs.com/skyfsm/p/6894685.html]
 ...
 protected:
  ...
  // OTE 作者不地道啊，这里是类的成员变量，说好的变量名的m前缀呢？
